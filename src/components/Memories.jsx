...
import memory1 from '../assets/memories11.png';
import memory2 from '../assets/memories22.png';
import memory3 from '../assets/memories33.png';

const Memories = () => {
  const memories = [
    { id: 1, title: "Как найти подход к конфликтным клиентам", image: memory1 },
    { id: 2, title: "Как создать шаблон для Email-рассылок в Контакт-Центре?", image: memory2 },
    { id: 3, title: "Как создать аутсорсинговый контакт-центр?", image: memory3 },
  ];
  ...
  return (
    <div className='px-4 lg:px-14 max-w-screen-2xl mx-auto my-12'>
      ...
      <div className='grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 items-center justify-between'>
        {memories.map(memory => (
          <div key={memory.id} className='mx-auto relative mb-12 cursor-pointer'>
            <img src={memory.image} alt={memory.title} className='hover:scale-95 transition-all duration-300 w-full h-48 object-cover rounded-lg' />
            <div className='text-center px-4 py-8 bg-white shadow-lg rounded md:w-3/4 mx-auto absolute left-0 right-0 -bottom-12'>
              <h3 className='mb-3 text-neutralGrey font-semibold'>{memory.title}</h3>
